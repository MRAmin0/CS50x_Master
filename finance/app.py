...
def index():
    ...
    try:
        for stock in portfolio:
            ...
            # update stuck
            stuck.update({"current_price": current_price, "stock_value": stock_value})
...
def buy():
    ...
    if request.method == "POST":
        ...
        if not shares.isdigit():
            return apology("Share should be a positive number !")
        transaction_value = int(shares) * stock["price"]
        user_cash = db.execute("SELECT cash FROM users WHERE id = ?", user_id)
        user_cash = user_cash[0]["cash"]
        # Does user have enoph cash?
        ...
        # work on database, add transaction value
        db.execute(
...
def withdraw():
    ...
    if request.method == "POST":
        ...
        # Ensure password is correct
        check_password(account[0]["hash"], request.form.get("password"))
        # Ensure user cannot withdraw more then left cash
